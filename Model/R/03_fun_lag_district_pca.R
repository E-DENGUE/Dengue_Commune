...
c1a<- c1a[,-c(17,18)]
  ...
  ##Y-AWARE PCA
      df2 <- c1b %>% dplyr::filter( !is.na( ttytv_tan_tru_la_log_df_lag4 ))
  ...
  form2 <- as.formula(paste0("obs_dengue_cases_hold ~", all.vars, " +lag3_monthly_cum_ppt + lag3_avg_daily_temp + sin12 +cos12 + f(t, model='ar1')"))
    ...
    form2 <- as.formula(paste0("obs_dengue_cases_hold ~", "lag3_monthly_cum_ppt + lag3_avg_daily_temp + sin12 +cos12 + f(t, model='ar1')"))
