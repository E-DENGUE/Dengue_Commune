...
c1a <- d2 %>%
  ...
  group_by(fcode) %>%
  mutate(fcode2=fcode,
         Dengue_fever_rates = obs_dengue_cases / pop *100000,
         log_df_lag2 = lag(log((obs_dengue_cases+0.5) / pop *100000),n=2),
         log_df_lag3 = lag(log((obs_dengue_cases+0.5) / pop *100000),n=3),
         log_df_lag4 = lag(log((obs_dengue_cases+0.5) / pop *100000),n=4),
         log_df_lag5 = lag(log((obs_dengue_cases+0.5) / pop *100000),n=5),
         sin12 = sin(2*pi*t/12),
         cos12 = cos(2*pi*t/12),
         fcodeID = fcode
...
##Y-AWARE PCA
df2 <- c1b %>% filter(date>='2010-06-01')

x <- as.data.frame(df2[,names(all.lags)])
...
y.aware.scale<- apply(x[,-1], 2, function(x1){
  ...
  x.scale<-x1*slope - mean(x1*slope, na.rm=T)
  return(x.scale)
})
...
form2 <- as.formula(paste0("obs_dengue_cases_hold ~",  " sin12 +cos12 + f(t, model='ar1')"))
...
scoring_func <- function(Y){
  ...
  in.ds <- Y$ds %>%
    arrange(date,fcodeID)  #SORTED AS MODEL DS IS SORTED
  ...
  obs_inc <- (c1$obs_dengue_cases/offset1)
  log_obs_inc <- log((c1$obs_dengue_cases+1)/offset1)
  
  miss.obs <- which(is.na(forecast_ds$obs_dengue_cases))
  ...
  out_ds <- forecast_ds %>%
    dplyr::select(date, fcode,forecast, index,  pop, obs_dengue_cases,horizon)%>%
...
c1.out <- c1 %>%
  dplyr::select(date, fcode, Dengue_fever_rates, obs_dengue_cases, forecast,horizon, offset1 ) 
